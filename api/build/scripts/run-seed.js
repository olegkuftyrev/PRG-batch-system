...
async function run() {
    ...
    for (const row of items) {
        await client.query(`INSERT INTO menu_items (code, title, station, batch_sizes, cook_times, enabled, recommended_batch, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())
       ON CONFLICT (code) DO UPDATE SET title=$2, station=$3, batch_sizes=$4, cook_times=$5, enabled=$6, recommended_batch=$7, updated_at=NOW()`, [row.code, row.title, row.station, JSON.stringify(row.batch_sizes), JSON.stringify(row.cook_times), row.enabled, JSON.stringify(row.recommended_batch)]);

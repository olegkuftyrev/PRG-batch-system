...
export function CallFoodItemWide({
  item,
  onCall,
  onCancel,
  onPriority,
  ...
  else if (isQualityCheck) buttonText = 'Almost done'
  else if (isCooking) {
  ...
  let cardClassName = 'flex flex-row overflow-hidden'
  if (isHoldExpired) cardClassName += ' border-4 border-red-600 bg-red-50'
  else if (isHoldExpiring) cardClassName += ' border-2 border-red-500 animate-pulse'
  ...
  return (
    <Card className={cardClassName}>
        ...
        <div className="px-4 pt-4 pb-2">
          <div className="flex items-center gap-3 mb-1">
            <div className="flex flex-col items-center">
              <span className="font-bold text-xl leading-tight">{item.code}</span>
          ...
          <h3 className="text-base font-semibold uppercase tracking-wide">{item.title}</h3>
        </div>
        <div className="flex-1 px-4 pb-4">
          {imageUrl ? (
            <img src={imageUrl} alt={item.title} className="w-full h-full object-cover rounded" />
          ) : (
      ...
      <CardContent className="flex flex-col justify-between w-1/2 p-4 gap-4">
            ...
            <div className="flex gap-1 justify-center flex-wrap">
              {item.batchSizes.map((size) => (
                <button
                  ...
                  disabled={disabled}
                  className={`px-4 py-2 min-w-[44px] rounded-md font-medium transition-all focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50 ${
                    batchSize === size ? 'bg-primary text-primary-foreground shadow-sm' : 'bg-muted text-muted-foreground hover:bg-muted/80'
                  }`}
                >
                  {size}
                </button>
          ...
          {(isCooking || isQualityCheck) ? (
            <div className="flex gap-2 items-center">
              {activeTicketId && onPriority && (
                <Button variant="outline" size="icon" className="h-12 w-12 shrink-0" onClick={handlePriority}>
              ...
              {activeTicketId && onCancel && (
                <Button variant="outline" size="icon" className="h-12 w-12 shrink-0" onClick={() => setShowCancelDialog(true)} disabled={canceling}>
            ...
            <div className="flex gap-2 items-center">
              ...
              <Button className="flex-1 h-12 text-base font-semibold" onClick={handleCall} disabled={loading || disabled}>
                {buttonText}
              </Button>
              {activeTicketId && onCancel && (
                <Button variant="outline" size="icon" className="h-12 w-12 shrink-0" onClick={() => setShowCancelDialog(true)} disabled={canceling}>
      ...
      <Dialog open={showInfo} onOpenChange={setShowInfo}>
        <DialogContent className="max-w-sm p-0 overflow-hidden rounded-2xl bg-white text-gray-900">
          ...
          <div className="overflow-y-auto max-h-[80vh] [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
            {imageUrl && <img src={imageUrl} alt={item.title} className="w-full object-cover" />}
